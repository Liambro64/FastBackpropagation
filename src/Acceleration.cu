...
{
	ddd *h_matrix = new ddd [matrix.size()];
	for (size_t i = 0; i < matrix.size(); ++i)
	{
		for (size_t j = 0; j < matrix[i].size(); ++j)
		{
			h_matrix[i * matrix[0].size() + j] = matrix[i][j];
		}
	}
	ddd *d_vector;
	ddd *returnVals;
	ddd *d_matrix;
	size_t pitch;
	if (cudaMalloc(&d_vector, vector.size() * sizeof(ddd)) != CUDA_SUCCESS)
		std::cout << "Failed to alloc 1" << std::endl;
	if (cudaMemcpy(d_vector, vector.data(), vector.size() * sizeof(ddd), cudaMemcpyHostToDevice) != CUDA_SUCCESS)
		std::cout << "Failed to copy 1" << std::endl;
	if (cudaMallocPitch(&d_matrix, &pitch, matrix[0].size() * sizeof(ddd), matrix.size()) != CUDA_SUCCESS)
		std::cout << "Failed to alloc (pitch) 1" << std::endl;
	if (cudaMemcpy2D(d_matrix, pitch, h_matrix, matrix[0].size() * sizeof(ddd),
			matrix[0].size() * sizeof(ddd), matrix.size(), cudaMemcpyHostToDevice) != CUDA_SUCCESS)
		std::cout << "Failed to copy 2" << std::endl;
	if (cudaMalloc(&returnVals, vector.size() * sizeof(ddd)) != CUDA_SUCCESS)
		std::cout << "Failed to alloc 3" << std::endl;
	cudaError_t err = cudaGetLastError();
	...
	dim3 blockSize = dim3(threadsPerBlock, 1, 1);
	dim3 gridSize = dim3(blocksPerGrid, 1, 1);
	vectorMatrixMultiplyGPU<<<gridSize, blockSize>>>(returnVals, d_vector, d_matrix, vsize, mrows, pitch / sizeof(ddd));

	cudaDeviceSynchronize();
	vec<ddd> result(vector.size());

	cudaMemcpy(result.data(), returnVals, vector.size(), cudaMemcpyDeviceToHost);
	cudaFree(d_vector);
	cudaFree(d_matrix);
	cudaFree(returnVals);
	delete h_matrix;
...
{
	double *h_c = new double[a.size() * b.size()];
	double *d_a;
	double *d_b;
	double **returnVals;
	size_t pitch;
	if (cudaMalloc((void **)&d_a, a.size() * sizeof(double)) != CUDA_SUCCESS)
		std::cout << "Failed to alloc 1" << std::endl;
	if (cudaMemcpy(d_a, a.data(), a.size() * sizeof(double), cudaMemcpyHostToDevice) != CUDA_SUCCESS)
		std::cout << "Failed to copy 1" << std::endl;
	if (cudaMalloc((void **)&d_b, b.size() * sizeof(double)) != CUDA_SUCCESS)
		std::cout << "Failed to alloc 2" << std::endl;
	if (cudaMemcpy(d_b, b.data(), b.size() * sizeof(double), cudaMemcpyHostToDevice) != CUDA_SUCCESS)
		std::cout << "Failed to copy 2" << std::endl;
	if (cudaMallocPitch((void **)&returnVals, &pitch, b.size() * sizeof(double), a.size()) != CUDA_SUCCESS)
		std::cout << "Failed to alloc 3" << std::endl;
	int asize = a.size();
	int bsize = b.size();
	int threadsPerBlock = 256;
	int blocksPerGrid = (asize + threadsPerBlock - 1) / threadsPerBlock;
	dim3 blockSize = dim3(threadsPerBlock, 1, 1);
	dim3 gridSize = dim3(blocksPerGrid, 1, 1);
	outerProductGPU<<<gridSize, blockSize>>>(returnVals, d_a, d_b, asize, bsize);
	cudaDeviceSynchronize();
	vec<vec<ddd>> result(a.size(), vec<ddd>(b.size()));

	cudaMemcpy2D(h_c, b.size() * sizeof(ddd), returnVals, pitch,
				 b.size() * sizeof(ddd), a.size(), cudaMemcpyDeviceToHost);
	for (int i = 0; i < a.size(); i++)
	{
		for (int j = 0; j < b.size(); j++)
		{
			result[i][j] = h_c[i * b.size() + j];
		}
	}
	cudaFree(d_a);
	cudaFree(d_b);
